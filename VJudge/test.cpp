...
line zhi[maxk];
circle c[maxk];
int nzhi,nshe,nduan,nc,nxian;

void input()
{
    scanf("%d%d%d%d", &nzhi,&nshe,&nduan,&nc);
    for(int i=0; i<nzhi; ++i) {
        zhi[i].a.input();
        zhi[i].b.input();
        zhi[i].id=1;
    }
    for(int i=nzhi; i<nzhi+nshe; ++i) {
        zhi[i].a.input();
        zhi[i].b.input();
        zhi[i].id=2;
    }
    for(int i=nzhi+nshe; i<nzhi+nshe+nduan; ++i) {
        zhi[i].a.input();
        zhi[i].b.input();
        zhi[i].id=3;
    }
    nxian = nzhi+nshe+nduan;
    for(int i=0; i<nc; ++i) {
        c[i].p.input();
        scanf("%lf", &c[i].r);
    }
        ...
        for(int j=0; j<nxian; ++j) {
            if(!zhi[i].parallel(zhi[j])) {
                ...
                if(zhi[i].id==2&&zhi[j].id==2){
                    ...
                    if(zhi[i].a==zhi[j].a&&zhi[i].b!=zhi[j].b) ans.insert(zhi[i].a);
                    if(zhi[i].b==zhi[j].b&&zhi[i].a!=zhi[j].a) ans.insert(zhi[i].b);
                }
            }
        }
    }
    for(int i=0; i<nxian; ++i) {
        for(int j=0; j<nc; ++j) {
            if(c[i].relationline(zhi[j])!=0) {
                ...
                else if(zhi[j].id==1&&flag==2)  ans.insert(tmp1),ans.insert(tmp2);

...
void input()
{
    scanf("%d%d%d%d", &nduan,&nshe,&nzhi,&nc);
    for(int i=0; i<nduan; ++i) {
        zhi[i].a.input();
        zhi[i].b.input();
        zhi[i].id=3;
    }
    for(int i=nduan; i<nduan+nshe; ++i) {
        zhi[i].a.input();
        zhi[i].b.input();
        zhi[i].id=2;
    }
    for(int i=nduan+nshe; i<nzhi+nshe+nduan; ++i) {
        zhi[i].a.input();
        zhi[i].b.input();
        zhi[i].id=1;
    }
...
int main()
{
    ...
    for(int i=0; i<nc; ++i) {
        for(int j=0; j<nc; ++j) {
            if(i!=j) {
                int flag =c[i].relationcircle(c[j]);
                if(flag>=2&&flag<=4) {
                    point tmp1, tmp2;
